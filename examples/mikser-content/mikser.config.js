...
//  - cors is on by default so the Vue dev servers on different ports can
//    fetch + subscribe without extra config
...
export default {
    plugins: [
        ...
        yaml(),
        layouts({
            // One generic page.html.hbs catches every document regardless
            // of meta.layout. The Vue apps do their own per-component
            // dispatch via meta.component; mikser's rendering is only
        ...
        schemas({
            ...
            // example projects — see e.g. hybrid-ssg/src/router.js for
            // how it's imported.
        ...
        data({
            ...
            // catalog and entity that gets configured here lands as a
            // JSON file under out/data/, served by the built-in static
            // handler. CDN-cacheable, survives the engine being down, and
            // the SDK reads from these files first before falling back to
            // the API.
            catalog: {
            ...
            entities: {
                // out/data/<entity.name>.page.json — one file per
                // published document, with full content. Consumed by the
                // SDK via
                //   entities('public', { data: { entities: 'page' } })
                // so useDocument(id) reads from disk on first paint
                // instead of hitting the API. Live updates still flow
                // over SSE.
                page: {
        ...
        api({
            endpoints: {
                ...
                // reading a document keeps reading it. Initial route data
                // comes from the static sitemap.json (above), not from
                // this endpoint.
                public: {
                    query: e => e.type === 'document' && e.meta?.published,
